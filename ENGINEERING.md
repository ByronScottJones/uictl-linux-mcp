...
screenshot/displays/pixel/OCR/clipboard (Phase 4 — screenshot/
displays.list/pixel/ocr/clipboard.get/clipboard.set implemented and
live-verified; only `wait-for` and ScreenCast+PipeWire (a hypothetical
zero-dialog screenshot follow-up) remain — see "Wayland screenshot"
section above)**, feedback + activity log GUI,
